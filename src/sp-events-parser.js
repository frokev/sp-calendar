  ...
  parseEvent: function(e, start, end) {
    if (!e.fRecurrence) {
      ...
      if (e.RecurrenceData.indexOf("<repeatInstances>") != -1) {
        rTotal = e.RecurrenceData.substring(
          e.RecurrenceData.indexOf("<repeatInstances>") + 17
        );
        rTotal = parseInt(rTotal.substring(0, rTotal.indexOf("<")));
      }
      if (e.RecurrenceData.indexOf("<daily ") != -1) {
        var str = e.RecurrenceData.substring(
          e.RecurrenceData.indexOf("<daily ")
        );
        ...
        if (arr.indexOf("dayFrequency") != -1) {
          ...
          e.RecurrenceData =
            e.RecurrenceData +
            "<weekly mo='TRUE' tu='TRUE' we='TRUE' th='TRUE' fr='TRUE' weekFrequency='1' />"; //change from daily on every weekday to weekly on every weekday
        }
      }
      if (e.RecurrenceData.indexOf("<weekly ") != -1) {
        var str = e.RecurrenceData.substring(
          e.RecurrenceData.indexOf("<weekly ")
        );
      ...
      if (e.RecurrenceData.indexOf("<monthly ") != -1) {
        var str = e.RecurrenceData.substring(
          e.RecurrenceData.indexOf("<monthly ")
        );
      ...
      if (e.RecurrenceData.indexOf("<monthlyByDay ") != -1) {
        var str = e.RecurrenceData.substring(
          e.RecurrenceData.indexOf("<monthlyByDay ")
        );
      ...
      if (e.RecurrenceData.indexOf("<yearly ") != -1) {
        var str = e.RecurrenceData.substring(
          e.RecurrenceData.indexOf("<yearly ")
        );
        str = str.substring(8, str.indexOf("/>") - 1);
        var arr = this.formatString(str);
      ...
      if (e.RecurrenceData.indexOf("<yearlyByDay ") != -1) {
        var str = e.RecurrenceData.substring(
          e.RecurrenceData.indexOf("<yearlyByDay ")
        );
